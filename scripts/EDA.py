...
def plot_station(df, station_id, station_name, suffix=""):
    ...
    plt.plot(df["FECHA"], df["LAeqDiurno"], label="LAeq Diurno", color='tab:orange', alpha=0.8)
    # plt.plot(df["FECHA"], df["LAeqNocturno"], label="LAeq Nocturno", color='tab:blue', alpha=0.8)
    
    plt.ylim(30, 105) # Fixed vertical scale for all plots

    title_extra = " (Desde 2022)" if suffix == "2022" else ""
    plt.xlabel("Fecha")
    plt.ylabel("Decibeles (dB)")
    plt.title(f"Evolución Diaria Nivel de Ruido Diurno: {station_name}{title_extra}")
    plt.legend()
    ...
    filename = f"station_{station_id}_{suffix}.png" if suffix else f"station_{station_id}_diurno.png"
